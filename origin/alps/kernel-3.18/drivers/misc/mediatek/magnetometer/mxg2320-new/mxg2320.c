...
//#define GPIO_MSENSOR_RST_EN      (GPIO61 | 0x80000000)
...
struct mag_hw mxg2320_cust;
static struct mag_hw *hw = &mxg2320_cust;

/* For  driver get cust info */
struct mag_hw *mxg2320_get_cust_mag(void)
{
	return &mxg2320_cust;
}
...
static int shipmenttestFlag = 0;////1==>shipmenttest, 0==>Not shipmenttest.
static int dataForMSensorXYZ[3]={0,0,0};
...
static int mxg2320_ReadPostureData(char *buf, int bufsize)
{
        ...
        read_lock(&data->mdata.datalock);
        sprintf(buf, "%x %x %x %x %x", data->mdata.ori.azimuth, data->mdata.ori.pitch,
                data->mdata.ori.roll, data->mdata.ori.status, CONVERT_FUSION_DIV);
        read_unlock(&data->mdata.datalock);
	...
	if ( mode == MXG2320_MOD_SELF_TEST ) {
        ...
        if ( mode == MXG2320_MOD_SNG_MEASURE ) {
                ...
                for ( i = 0; i < 3; i++ ) {
						...
						dataForMSensorXYZ[i] = data->mdata.umag.data[i];
                }
...
static ssize_t show_shipmenttest_value(struct device_driver *ddri, char *buf)
{
    char result[10];
		...
		if(1 == ret)
		{
		   MSE_ERR("shipment_test pass\n");
		   strcpy(result,"y");
		}
		else if(-1 == ret)
		{
		   MSE_ERR("shipment_test fail\n");
		   strcpy(result,"n");
		}
		else
		{
		  MSE_ERR("shipment_test NaN\n");
		  strcpy(result,"NaN");
		}
        return sprintf(buf, "%s\n", result);
}
...
static DRIVER_ATTR(shipmenttest,     S_IRUGO | S_IWUSR, show_shipmenttest_value, store_shipment_test);
...
static long mxg2320_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if ( argp == NULL ) {
        ...
        case MSENSOR_IOCTL_SET_CALIDATA:
                ...
                data->mdata.umag.oy    = calibrated_data[10];
                data->mdata.umag.oz    = calibrated_data[11]; 
                ...
                if ( copy_from_user(&status, argp, sizeof(status)) ) {
                        ...
                        if ( status ) {
                	        write_lock(&data->mcrtl.ctrllock);
								...
								data->mcrtl.active[MX_SEN_ORIENTATION] = 1;////add by James.
                                data->mcrtl.delay[MX_SEN_MAGNETIC] = 100;////add by James.
								data->mcrtl.delay[MX_SEN_ORIENTATION] = 100;////add by James.
                                write_unlock(&data->mcrtl.ctrllock);
                		...
                		atomic_set(&data->mcrtl.open_flag, 1);
                	        ...
                	        write_lock(&data->mcrtl.ctrllock);
								...
								data->mcrtl.active[MX_SEN_ORIENTATION] = 0;////add by James.
                                data->mcrtl.delay[MX_SEN_MAGNETIC] = -1;
								data->mcrtl.delay[MX_SEN_ORIENTATION] = -1;////add by James.
                                write_unlock(&data->mcrtl.ctrllock);
        ...
        case MSENSOR_IOCTL_SNG_MEASURE:
                ...
                break;
        case MSENSOR_IOCTL_DAEMON_READ_SENSORDATA:
			MSE_ERR("MSENSOR_IOCTL_DAEMON_READ_SENSORDATA\n");
			if(shipmenttestFlag == 0)
			{
			////no need mxg2320_operation_mode() now...2015.11.24.
			////mxg2320_operation_mode(MXG2320_MOD_SNG_MEASURE);////add by James.2015.
			...
			sprintf(buf, "%x %x %x", raw_data[0], raw_data[1], raw_data[2]);
			if (copy_to_user(argp, buf, strlen(buf)+1))
			{
					return -EFAULT;
			}
        ...
        case MSENSOR_IOCTL_READ_SENSORDATA:
                MSE_ERR("MSENSOR_IOCTL_READ_SENSORDATA, shipmenttestFlag=%d.\n", shipmenttestFlag);
				if(shipmenttestFlag == 0)
				{
                    mxg2320_operation_mode(MXG2320_MOD_SNG_MEASURE);////add by James.2015.
					msleep(10);////try to delay 10ms as IC need 10ms to ready the XYZ Data.
                    mxg2320_get_raw_data(MXG2320_MOD_SNG_MEASURE);
                //read_lock(&data->mdata.datalock);//JamesNoNeedThisLock.
		        raw_data[0] = dataForMSensorXYZ[0];//data->mdata.umag.x * CONVERT_M;
		        raw_data[1] = dataForMSensorXYZ[1];//data->mdata.umag.y * CONVERT_M;
		        raw_data[2] = dataForMSensorXYZ[2];//data->mdata.umag.z * CONVERT_M;
		        ////*status = data->mdata.mag.status;////because MTK ftm_msensor.c not need status now.
		        //read_unlock(&data->mdata.datalock);
				}else
				{
				    read_lock(&data->mdata.datalock);
		...
		void* buff_out, int size_out, int* actualout)
{
        ...
        case SENSOR_GET_DATA:
                ...
                if ( (buff_out == NULL) || (size_out< sizeof(struct hwm_sensor_data)) ) {
                        ...
                        msensor_data->status = data->mdata.mag.status;
                        read_unlock(&data->mdata.datalock);
                ...
                int size_in, void* buff_out, int size_out, int* actualout)
{
        int err = 0;
        int value;
        struct hwm_sensor_data* rvsensor_data;
        struct mxg2320_i2c_data *data = (struct mxg2320_i2c_data *)self;
                ...
                int size_in, void* buff_out, int size_out, int* actualout)
{
        int err = 0;
        int value;
        struct hwm_sensor_data* gvsensor_data;
        struct mxg2320_i2c_data *data = (struct mxg2320_i2c_data *)self;
                ...
                int size_in, void* buff_out, int size_out, int* actualout)
{
        int err = 0;
        int value;
        struct hwm_sensor_data* lasensor_data;
        struct mxg2320_i2c_data *data = (struct mxg2320_i2c_data *)self;
...
static int mxg2320_init_device(struct mxg2320_i2c_data *data)
{
        ...
        data->hw = mxg2320_get_cust_mag();
#if defined(LYCONFIG_AUTO_PLATFORM_NAME_F300C) || defined(LYCONFIG_AUTO_PLATFORM_NAME_F300) ||   defined(LYCONFIG_AUTO_PLATFORM_NAME_F300CS) || defined(LYCONFIG_AUTO_PLATFORM_NAME_F300S) 
	 data->hw->direction = 1;
#endif
...
extern void msensor_rst_output( int level);

static int mxg2320_local_init(void)
{
        struct mag_hw *hw = mxg2320_get_cust_mag();

		msensor_rst_output(0);
		mdelay(10);
        ...
        {
                MSE_ERR("mxg2320 add driver error\n");
                return -1;
        }
...
{
	const char *name = "mediatek,MXG2320";

	hw = get_mag_dts_func(name, hw);
	if (!hw)
		MSE_ERR("mxg2320 get dts info fail\n");

    mag_driver_add(&mxg2320_init_info);
		
  	MSE_INF("MXG2320 init new mtk architecture...\n");
