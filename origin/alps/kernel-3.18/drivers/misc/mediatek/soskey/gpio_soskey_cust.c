...
#ifdef CONFIG_OF
static const struct of_device_id hall_1_of_match[] = {
	...
	if (kpd_input_dev != NULL)
	{
		switch (state)
		{
			case 1:
				...
				input_report_key(kpd_input_dev, KEY_Q, 0);
				input_sync(kpd_input_dev);
				break;
			case 0:
				input_report_key(kpd_input_dev, KEY_Q, 1);
...
*/
	switch_data->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, HALL_1-eint");
	...
	if (switch_data->irq_node) {
		...
		ret = request_irq(switch_data->irq, gpio_irq_handler, IRQF_TRIGGER_NONE, "HALL_1-eint", switch_data);
		if (ret < 0) 
		{
...
static struct platform_driver gpio_switch_driver = {
	...
	.driver		= {
		.name	= "hall_1",
#ifdef CONFIG_OF
	 .of_match_table = hall_1_of_match,
#endif		
	},
};
