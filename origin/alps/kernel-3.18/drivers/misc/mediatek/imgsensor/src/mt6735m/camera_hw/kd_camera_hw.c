...
//add by xiehaifei 160413 for main camera  the same to sub camera 
#ifdef LYCONFIG_MAIN_CAM_THE_SAME_TO_SUB
unsigned int g_CurrentSearchSensorIdx=0;
#endif
...
CAMERA_POWER_INFO camera_d_vol_table[] = 
{
	...
	{SENSOR_DRVNAME_IMX175_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_OV5675_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_OV5648_MIPI_RAW, VOL_1500, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_OV5693_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_OV13850_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_OV9760_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_OV9760_MAIN_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_GC2355_MIPI_RAW, VOL_1800, GPIO_LOW_LEVEL},
	{SENSOR_DRVNAME_GC2365_MIPI_RAW, VOL_1800, GPIO_LOW_LEVEL},
	{SENSOR_DRVNAME_GC2375_MIPI_RAW, VOL_1800, GPIO_LOW_LEVEL},
	...
	{SENSOR_DRVNAME_GC5025_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},		//daili add 
	{SENSOR_DRVNAME_GC5005_MIPI_RAW, VOL_1200, GPIO_LOW_LEVEL},
	{SENSOR_DRVNAME_SP0A09_MIPI_RAW, VOL_1800, GPIO_LOW_LEVEL},
	{SENSOR_DRVNAME_SP2509_MIPI_RAW, VOL_1500, GPIO_LOW_LEVEL},
	...
	{SENSOR_DRVNAME_S5K3H7YX_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
	{SENSOR_DRVNAME_GC8024_MIPI_RAW, VOL_1200, GPIO_LOW_LEVEL},
	{SENSOR_DRVNAME_GC2385_MIPI_RAW, VOL_1200, GPIO_HIGH_LEVEL},
};
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAMPDN:
		...
		if (PinIdx == 0) {
			if (Val == 0)
			{
				#if defined(LYCONFIG_AUTO_PLATFORM_NAME_F9B) || defined(LYCONFIG_AUTO_PLATFORM_NAME_F9BS)  
...
bool _hwPowerOnCnt(KD_REGULATOR_TYPE_T powerId, int powerVolt/*, char *mode_name*/)
{
	...
	if (_hwPowerOn(powerId, powerVolt)) {
		...
		if (powerId == VCAMD)
			...
			cntVCAMAF += 1;
		else if (powerId == VSUBCAMD)
			cntVCAMD_SUB += 1;
		else if (powerId == VSUBCAMA)
			cntVCAMA_SUB += 1;
...
void checkPowerBeforClose(char *mode_name)
{
	...
	PK_DBG
	    ("[checkPowerBeforClose]cntVCAMD:%d, cntVCAMA:%d,cntVCAMIO:%d, cntVCAMAF:%d, cntVCAMD_SUB:%d,cntVCAMA_SUB:%d,\n",
	     cntVCAMD, cntVCAMA, cntVCAMIO, cntVCAMAF, cntVCAMD_SUB,cntVCAMA_SUB);
	...
	for (i = 0; i < cntVCAMD_SUB; i++)
		_hwPowerDown(VSUBCAMD);
	for (i = 0; i < cntVCAMA_SUB; i++)
		_hwPowerDown(VSUBCAMA);
...
/*LY 6735P Camera LDO CONFIG*/
/*LDO　　**BackCam**FrontCam*/
/*AVDD　**VCAMA　　**VCAMA　　　*/
/*DVDD　**VCAMD　　**VRF18_1　*/
/*IOVDD**VCAMIO　**VCAMIO　　*/
/*AFVCC**VCAMAF　**VCAMAF　　*/
		       ...
		       char *mode_name)
{
	...
	if (DUAL_CAMERA_MAIN_SENSOR == SensorIdx)
		...
		pinSetIdx = 2;
	#if defined(LYCONFIG_MAIN_CAM_THE_SAME_TO_SUB)
        g_CurrentSearchSensorIdx=pinSetIdx;    
        #endif
	...
	//liulinzhi add start
	if((pinSetIdx==0)&&(currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2375_MIPI_RAW, currSensorName))))
		return 0;
	//liulinzhi add ended
	...
	switch (SensorIdx)
	{
		case DUAL_CAMERA_MAIN_SENSOR:
		 	...
		 	if (On)
			{
				...
				//VCAM_A
				#if 0//defined(LYCONFIG_EXCHANGE_BACK_FRONT_CAMERA_SUPPORT)
		...
		case DUAL_CAMERA_SUB_SENSOR:
			if (On)
			{
				...
				//VCAM_A	
				#if 1//defined(LYCONFIG_EXCHANGE_BACK_FRONT_CAMERA_SUPPORT) 
				if(TRUE != _hwPowerOn(VCAMA, VOL_2800))
				{
					PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_A), power id = %d\n", VCAMA);
					goto _kdCISModulePowerOn_exit_;
				...
				printk("_hwPowerOn VSUBCAMD !!\n");
				if(TRUE != _hwPowerOn(VSUBCAMD, d_vol))
					{
						printk("[CAMERA SENSOR] Fail to enable digital power (VCAM_D), power id = %d \n", VCAMD);
						goto _kdCISModulePowerOn_exit_;
					}
				#endif
			    	mdelay(2);
