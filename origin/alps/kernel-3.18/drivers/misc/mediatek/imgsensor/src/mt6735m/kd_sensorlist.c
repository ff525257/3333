...
#define PROC_CAMERA_INFO "hw_info/camera_info"
#else
...
bool Get_Cam_Regulator(void)
{
	...
	if (1) {
		...
		if (node) {
			name = of_get_property(node, "vcama_sub-supply", NULL);
			name2 = of_get_property(node, "vcamd_sub-supply", NULL);
			if (name == NULL && name2==NULL) {
				  printk("<xhf>Get_Cam_Regulator-----only main camera!/n");
				if (regVCAMA == NULL) {
				...
				printk("<xhf>Get_Cam_Regulator-----dual  camera!/n");
...
bool _hwPowerOn(KD_REGULATOR_TYPE_T type, int powerVolt)
{
	...
	if (type == VCAMA) {
		...
		reg = regVCAMAF;
    } else if (type == VSUBCAMA) {
	reg = regSubVCAMA;	
    } else if (type == VSUBCAMD) {
	reg = regSubVCAMD;	
    } else
	return ret;
	...
	if (reg != NULL && !IS_ERR(reg)) {
		...
		if (regulator_enable(reg) != 0) {
			printk("[_hwPowerOn]fail to regulator_enable, powertype:%d powerId:%d\n",
			       type, powerVolt);
			return ret;
		}
		ret = true;
	} else {
		printk("[_hwPowerOn]IS_ERR_OR_NULL powertype:%d\n", type);
		return ret;
	}
