...
//wanggang add fingerpring-func-support start
import android.net.Uri;
import android.database.Cursor;
import android.content.ContentValues;
//wanggang add fingerpring-func-support end

//[CDFinger] Fingerprint support start
import android.telecom.TelecomManager;
//[CDFinger] Fingerprint support end
...
public class PhoneWindowManager implements WindowManagerPolicy {
    //[CDFinger] Fingerprint support start
    private static final boolean CDFINGER_SUPPORT = LyFeatureUtils.LYCONFIG_FEA_CDFINGER_FINGERPRINT;
    public static final String CDFINGER_FINGERPRINT_USEDTO_CAMERA = Settings.System.SHLY_CDFP_CAMERA;
    public static final String CDFINGER_FINGERPRINT_USEDTO_CALL = Settings.System.SHLY_CDFP_INCALL;
    public static final String CDFINGER_FINGERPRINT_USEDTO_DESKTOP = Settings.System.SHLY_CDFP_LAUNCHER;
    public static final String CDFINGER_FINGERPRINT_USEDTO_MUSIC = Settings.System.SHLY_CDFP_MUSIC;
    public static final String CDFINGER_FINGERPRINT_USEDTO_READER = "com_cdfinger_fingerprint_usedto_reader";

 
    private static final int MSG_FINGERPRINT_KEY = 18;
    private static final long FINGERPRINT_DEBOUNCE_DELAY_MILLIS = 500;
    private long   mFingerprintKeyTime = 0;
    //[CDFinger] Fingerprint support end
    ...
    //add by kevin for tp-gestures 20140730 start
    private static final boolean TP_GESTURES_SUPPORT = SystemProperties.getBoolean("shly_tpgestures_support", false);
    private static final boolean tpg_dbg = false;
    private static final String TPG_TAG = "tpgestures";
    private boolean mNeedHandleTpKey;
    private int mRecordTpKey = -1;
    //add by kevin for tp-gestures 20140730 end

     //add by xiangchangsong for double click on home off screen  20160602 start
     boolean mHomeDownDoubleClick = false; 
     long mLastHomeDownTime=0; 
     long mLastHomeUpTime=0;
     //add by xiangchangsong for double click on home off screen  20160602 end
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        //[add by Tauren 20141202 for three_touch_cutscreen start]
        // LYCONFIG_FEA_THREETOUCH_SCREENSHOT_SUPPORT
        filter = new IntentFilter("shly.intent.action.screenshot");
        context.registerReceiver(mShlyScreenShotReceiver, filter);
        //[add by Tauren 20141202 for three_touch_cutscreen end]
    ...
    public void setInitialDisplaySize(Display display, int width, int height, int density) {
        ...
        //add by kevin for controlling nav bar 20151027 start
        if (!LyFeatureUtils.LYCONFIG_FEA_NAVIGATION_BAR_SUPPORT) {
            mHasNavigationBar = false;
        }
        //add by kevin for controlling nav bar 20151027 end
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
           ...
           //jianglingfeng add for factory_mode_keycode_test_key_home not go home 20160105 start
           ...
           try{
                ...
                if (runningTaskInfos.size()> 0){
                        ...
                        final String className = info.topActivity.getClassName();
                        if (className != null && className.equals("com.shly.factorymode.KeyCode")) {
                                return 0;
                        }
                }
           }catch(Exception e){
           ...
           //jianglingfeng add for factory_mode_keycode_test_key_home not go home 20160105 end
            if (!down) {
...
//A: lanhua for keycode_camera 111217 start
	private static final String[] SPECIAL_ACTIVITYS=new String[]{
		"com.android.soundrecorder",
		"com.mediatek.camera",
		"com.android.incallui.InCallActivity",
		"com.shly.factorymode"
	};
	private boolean isCameraKeyLongPress = false;
	private boolean isCameraKeyDoubleTap =  false;
	private void startCameraKeyActivity(ComponentName component,String key,String value){
	...
	private boolean isSpecialActivity(){
		...
		try{
			...
			if (runningTaskInfos.size()> 0){
				RunningTaskInfo info = runningTaskInfos.get(0);
				String mPkgName = info.topActivity.getPackageName();
				String mClassName = info.topActivity.getClassName();
				
				for(int i=0;i<SPECIAL_ACTIVITYS.length;i++){
					String value = SPECIAL_ACTIVITYS[i];
					if(value.equals(mPkgName) || value.equals(mClassName)){
						isSpecial = true;
						break;
					}
	...
	private final Runnable mCameraKeyLongRunnable = new Runnable() {
        @Override
        public void run() {
        	isCameraKeyLongPress = true;
        	hanleCameraKeyLongPress();
        }
    };
   private final Runnable mCameraKeyDoubleRunnable = new Runnable() {
        @Override
        public void run() {
        	isCameraKeyDoubleTap = false;
        }
    };
    
    private void hanleCameraKeyDoublePress(){
		if(isScreenOn() && !keyguardOn() && !isSpecialActivity()){
			ComponentName component=new ComponentName("com.mediatek.camera", "com.android.camera.CameraLauncher");
			startCameraKeyActivity(component,null,null);
		}
    }
    
    private void hanleCameraKeyLongPress(){
		if(isScreenOn() && !keyguardOn() && !isSpecialActivity()){
			ComponentName component=new ComponentName("com.mediatek.camera", "com.android.camera.CameraLauncher");
			startCameraKeyActivity(component,"isStartRecorder","true");
		}
    }
	//A: lanhua for keycode_camera 111217 end
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        //add by lijun for smart-wakeup 20150115 start
        if (TP_GESTURES_SUPPORT) {
            setNeedHandleTpKey(false, -1);
            if (tpg_dbg) Log.d(TPG_TAG, "interceptKeyBeforeQueueing check tpkey keyCode:" + keyCode);
            final boolean isTpKey = checkIsTpKey(keyCode);
            if (isTpKey) {
                if (keyCode == KeyEvent.KEYCODE_M) {
                    ...
                    setNeedHandleTpKey(true, keyCode);
                }
            }
        }
        //add by lijun for smart-wakeup 20150115 end
	    ...
	    //[CDFinger] Fingerprint support start
            case KeyEvent.KEYCODE_F11: {
                if (CDFINGER_SUPPORT) {
                    if (down) {
                        Log.i(TAG, "KEYCODE_F11 event");
                        final long now = SystemClock.uptimeMillis();
                        final long timeoutTime = FINGERPRINT_DEBOUNCE_DELAY_MILLIS + mFingerprintKeyTime;
                        if (now > timeoutTime) {
                            Message msg = mHandler.obtainMessage(MSG_FINGERPRINT_KEY);
                            msg.setAsynchronous(true);
                            msg.sendToTarget();
                            mFingerprintKeyTime = now;
                        }
                       //accept Ringing Call
                        if (Settings.System.getInt(mContext.getContentResolver(), CDFINGER_FINGERPRINT_USEDTO_CALL, 0) == 1) {
            ...
            case KeyEvent.KEYCODE_POWER: {
             ...
             case KeyEvent.KEYCODE_HOME: {
                 if (LyFeatureUtils.LYCONFIG_FEA_DOUBLE_CLICK_HOME_OFF_SCREEN) {
                    boolean isEnableDoubleTapHome = Settings.System.getInt(mContext.getContentResolver(), Settings.System.SMARTGESTURE_ON_OFF, 0) != 0;
                    if (isEnableDoubleTapHome) {
                        isEnableDoubleTapHome = Settings.System.getInt(mContext.getContentResolver(), Settings.System.SMARTGESTURE_DOUBLE_TAP_HOME, 0) != 0;
                    }
 
                     if (isEnableDoubleTapHome && isScreenOn()) {
                         if (down) {
                             if(((event.getEventTime() - mLastHomeDownTime) < 500)){ 
                                 mHomeDownDoubleClick = true; 
                            }else{
                                 mHomeDownDoubleClick = false;
                             }
                             mLastHomeDownTime = event.getEventTime();                              
                         } else {
                             if(mHomeDownDoubleClick && ((event.getEventTime() - mLastHomeUpTime) < 500)){ 
                                 mPowerManager.goToSleep(SystemClock.uptimeMillis());
                                 PowerManager.WakeLock homeKeyWakeLock = mPowerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
                                 "PhoneWindowManager.homeKeyWakeLock");
                                 homeKeyWakeLock.acquire();
                                 homeKeyWakeLock.release();
                             }
                             mLastHomeUpTime = event.getEventTime();                            
                         }
                     }
                 }
                 break;
            ...
            //add by jianglingfeng for hall 20151211 start
            case KeyEvent.KEYCODE_F9:
                if(LyFeatureUtils.LYCONFIG_MTK_FLIP_HALL_SUPPORT  && down){
                    result &= ~ACTION_PASS_TO_USER;
                    isWakeKey = false;
                    if(LyFeatureUtils.LYCONFIG_FEA_HALL_VIEW_SUPPORT != 0){
                    	mKeyguardDelegate.hallFlipClose();
                    }else{
						mPowerManager.goToSleep(event.getEventTime(),
                    	PowerManager.GO_TO_SLEEP_REASON_POWER_BUTTON, 0);
                    }
            ...
            case KeyEvent.KEYCODE_F10:
                if(LyFeatureUtils.LYCONFIG_MTK_FLIP_HALL_SUPPORT && down){
                    ...
                    isWakeKey = true;
                     if(LyFeatureUtils.LYCONFIG_FEA_HALL_VIEW_SUPPORT != 0){
                     	mKeyguardDelegate.hallFlipOpen();
                     }else{
						final boolean isSecure0 = isKeyguardSecure();
                    	if(!isSecure0){
                ...
                break;
            //add by jianglingfeng for hall 20151211 end
			
			case KeyEvent.KEYCODE_Q:
                
		        	if(down){
					    mHandler.removeCallbacks(mCameraKeyLongRunnable);
		        		mHandler.postDelayed(mCameraKeyLongRunnable, 600);
		        	}else{
		        		mHandler.removeCallbacks(mCameraKeyLongRunnable);
		        		if(!isCameraKeyLongPress){
		        			if(isCameraKeyDoubleTap){
		        				mHandler.removeCallbacks(mCameraKeyDoubleRunnable);
		        				isCameraKeyDoubleTap = false;
		        				hanleCameraKeyDoublePress();
		        			}else{
		        				isCameraKeyDoubleTap = true;
		        				mHandler.postDelayed(mCameraKeyDoubleRunnable, 500);
		        			}
		        		}else{
		        			isCameraKeyDoubleTap = false;
		        		}
		        		isCameraKeyLongPress = false;
		        	}
    ...
    //add by kevin for super screenshot 20160518 start
    private BroadcastReceiver mShlyScreenShotReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if ("shly.intent.action.screenshot".equals(intent.getAction())) {
                mHandler.removeCallbacks(mScreenshotRunnable);
                mHandler.postDelayed(mScreenshotRunnable, 500);
            }
        }
    };        
    //add by kevin for super screenshot 20160518 end  
    ...
    public void screenTurnedOff() {
        ...
        //wanggang add fingerpring-func-support start
        if(LyFeatureUtils.LYCONFIG_FEA_FINGERPRINT_MORE_FUNC_SUPPORT && mSystemReady){
            mHandler.post(mAppLockRunnable);
        }
        //wanggang add fingerpring-func-support end
    ...
    public void screenTurningOn(final ScreenOnListener screenOnListener) {
        ...
        synchronized (mLock) {
            ...
            if (mKeyguardDelegate != null) {
            //add by kevin for tp gesture 20151208 start
            if (TP_GESTURES_SUPPORT) {
                if (mNeedHandleTpKey && mRecordTpKey != -1) {
                    handleTpKey(mRecordTpKey);
                }
            }
            if (tpg_dbg) Log.d(TPG_TAG, "wakingUp");
            //add by kevin for tp gesture 20151208 end
                mHandler.removeMessages(MSG_KEYGUARD_DRAWN_TIMEOUT);
   ...
   ///@}
    ...
    //add by kevin for tp-gestures 20140730 start
    private class TpKeyInfo {
        int mKeyCode;
        String mKey;
        
        public TpKeyInfo(int keyCode, String key) {
            this.mKeyCode = keyCode;
            this.mKey = key;
        }
    }
    
    private TpKeyInfo[] mTpKeyInfo = new TpKeyInfo[]{
            new TpKeyInfo(KeyEvent.KEYCODE_DPAD_UP, Settings.System.SMARTGESTURE_UP),
            new TpKeyInfo(KeyEvent.KEYCODE_DPAD_DOWN, Settings.System.SMARTGESTURE_DOWN),
            new TpKeyInfo(KeyEvent.KEYCODE_DPAD_RIGHT, Settings.System.SMARTGESTURE_DRAW_RIGHT),
            new TpKeyInfo(KeyEvent.KEYCODE_DPAD_LEFT, Settings.System.SMARTGESTURE_DRAW_LEFT),
            new TpKeyInfo(KeyEvent.KEYCODE_W, Settings.System.SMARTGESTURE_DRAW_W),
            new TpKeyInfo(KeyEvent.KEYCODE_C, Settings.System.SMARTGESTURE_DRAW_C),
            new TpKeyInfo(KeyEvent.KEYCODE_E, Settings.System.SMARTGESTURE_DRAW_E),
            new TpKeyInfo(KeyEvent.KEYCODE_O, Settings.System.SMARTGESTURE_DRAW_O),
            new TpKeyInfo(KeyEvent.KEYCODE_M, Settings.System.SMARTGESTURE_DRAW_M),
            new TpKeyInfo(KeyEvent.KEYCODE_S, Settings.System.SMARTGESTURE_DRAW_S),
            new TpKeyInfo(KeyEvent.KEYCODE_V, Settings.System.SMARTGESTURE_DRAW_V),
            new TpKeyInfo(KeyEvent.KEYCODE_P, Settings.System.SMARTGESTURE_DOUBLE_CLICK)
        };     
    
    private boolean checkIsTpKey (int keyCode) {
        boolean isTpKey = false;
        String keyValue;
        if ((Settings.System.getInt(mContext.getContentResolver(), Settings.System.SMARTGESTURE_ON_OFF, 0) == 0)) {
            return false;
        }
        
        for (int i=0; i<mTpKeyInfo.length; i++) {
            if (keyCode == mTpKeyInfo[i].mKeyCode) {
                keyValue = mTpKeyInfo[i].mKey;
                isTpKey = (Settings.System.getInt(mContext.getContentResolver(), keyValue, 0) != 0);
                if (tpg_dbg) Log.d(TPG_TAG, "checkIsTpKey keyValue:" + keyValue); 
                break;
            }
        }
    ...
    private void handleTpKey (int keyCode) {
                  
        if (keyCode == KeyEvent.KEYCODE_P || keyCode == KeyEvent.KEYCODE_DPAD_RIGHT || keyCode == KeyEvent.KEYCODE_DPAD_LEFT
                || keyCode == KeyEvent.KEYCODE_M) {
            return;
        }
        if (mKeyguardDelegate != null) {
            mKeyguardDelegate.setTpKeyCode(keyCode);
        }
        if (tpg_dbg) Log.d(TPG_TAG, "handleTpKey keyCode: " + keyCode);                              
    }   
    private void setNeedHandleTpKey(boolean need, int recordTpKey) {
        mNeedHandleTpKey = need;
        mRecordTpKey = recordTpKey;
    } 
    //add by kevin for tp-gestures 20140730 end
    ...
    /// @}
    //wanggang add fingerpring-func-support start
    private Runnable mAppLockRunnable = new Runnable(){
        public void run(){
            ...
            if(c != null){
                while (c.moveToNext()) {
                   ...
                   values.put("packagename", c.getString(c.getColumnIndexOrThrow("packagename")));
                   values.put("tid", c.getInt(c.getColumnIndexOrThrow("tid")));
                   values.put("lock", 0);
                   String updateToWhere = "tid" + "=" + c.getInt(c.getColumnIndexOrThrow("tid"));
 ...
 //[CDFinger] Fingerprint support start
    void TranasferFingerprintKey() {
        ...
        else if (isReaderAppLaunch()) validApp = 3;
        else if (isLauncherAppLaunch()) validApp = 4;
        ...
        if (isFunctionEnable(validApp)) {
            ...
            if (validApp == 1) {
                keyRemappingSendFakeKeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_CAMERA);
                keyRemappingSendFakeKeyEvent(KeyEvent.ACTION_UP, KeyEvent.KEYCODE_CAMERA);
            } else if (validApp == 2) {
                keyRemappingSendFakeKeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_MEDIA_NEXT);
                keyRemappingSendFakeKeyEvent(KeyEvent.ACTION_UP, KeyEvent.KEYCODE_MEDIA_NEXT);
            } else if (validApp == 3) {
                ...
                keyRemappingSendFakeKeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_F12);
                keyRemappingSendFakeKeyEvent(KeyEvent.ACTION_UP, KeyEvent.KEYCODE_F12);
            }
        }
    }
    ...
    private boolean isReaderAppLaunch() {
        ...
        for (int i = 0; i < mReaderApps.length; i++) {
            if (topPackageName != null && mReaderApps[i].contains(topPackageName)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isLauncherAppLaunch() {
        String topPackageName = getTopActivity().getPackageName();
        for (int i = 0; i < mLauncherApps.length; i++) {
            if (topPackageName != null && mLauncherApps[i].contains(topPackageName)) {
                return true;
            }
        }
          return false;
    ...
    private boolean isFunctionEnable(int functionID) {
        ...
        if (functionID == 1) {// camera
            fEnable = Settings.System.getInt(mContext.getContentResolver(), CDFINGER_FINGERPRINT_USEDTO_CAMERA, 0) == 1 ? true : false;
        } else if (functionID == 2) {//music
            fEnable = Settings.System.getInt(mContext.getContentResolver(), CDFINGER_FINGERPRINT_USEDTO_MUSIC, 0) == 1 ? true : false;
        } else if (functionID == 3) {//reader
            fEnable = Settings.System.getInt(mContext.getContentResolver(), CDFINGER_FINGERPRINT_USEDTO_READER, 0) == 1 ? true : false;
        } else if (functionID == 4) {
            fEnable = Settings.System.getInt(mContext.getContentResolver(), CDFINGER_FINGERPRINT_USEDTO_DESKTOP, 0) == 1 ? true : false;
        }
        return fEnable;
    }
    //[CDFinger] Fingerprint support end
