...
import com.android.systemui.statusbar.phone.HallView;
...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    protected static final int MSG_HALL_FLIP_CLOSE = 3001;
    protected static final int MSG_HALL_FLIP_OPEN = 3002;
    
    //A: lanhua for hall_view 160422 start
    ...
    //A: lanhua for hall_view 160422 end
    ...
    protected HallView mHallView = null;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_USER_SWITCHED.equals(action)) {
              ...
              //A: lanhua for hall_view 160422 start
            	...
            	isAlarmRing = true;
            	updateHallView();
            }else if(ALARM_DISMISS_ACTION.equals(action) 
            		|| ALARM_DONE_ACTION.equals(action) 
            		|| ALARM_SNOOZE_ACTION.equals(action)){
            	isAlarmRing = false;
            	updateHallView();
            }
          //A: lanhua for hall_view 160422 end
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(sbn.getPackageName() != null
							&& (sbn.getPackageName().equals("com.shly.factorymode"))
							&& (sbn.getId() == 0x27)
							){
							return;
                        }
    ...
    protected class H extends Handler {
        public void handleMessage(Message m) {
            ...
            //wanggang add for hallFlip 0413 start
            case MSG_HALL_FLIP_CLOSE:
                addHallView();
                break;
            case MSG_HALL_FLIP_OPEN:
                removeHallView();
                break;
            }
            //wanggang add for hallFlip 0413 end
        }
    }

    //wanggang add for hallFlip 0413 start
    private void updateHallView(){
    	if(mHallView != null && mHallView.getParent() != null){
    		mHallView.setAlarmRing(isAlarmRing);
    		mHallView.setCallState(isRing,isCalling, number);
    		mHallView.gotoFirstPage();
    	//	mWindowManager.updateViewLayout(mHallView, mHallView.getLayoutParams());
    	}	
    }
    
    private void addHallView() {
        if (mHallView == null) {
            mHallView = (HallView) View.inflate(mContext, R.layout.shly_hall, null);
        }
        mHallView.setLayoutDirection(View.LAYOUT_DIRECTION_LTR );
		mHallView.setAlarmRing(isAlarmRing);
		mHallView.setCallState(isRing,isCalling, number);
		if(mHallView.getParent() == null){
        	mWindowManager.addView(mHallView, getNavigationBarLayoutParams());
        }
    }
    private void removeHallView() {
        if (mHallView == null) return;
        if(mHallView.getParent() != null){
    ...
    protected boolean inflateViews(Entry entry, ViewGroup parent) {
 	...
 	// wanggang add for notifyoverlaySecurity 0528 start
    public void closePanel() {
    }
    // wanggang add for notifyoverlaySecurity 0528 end
    ...
    private final class NotificationClicker implements View.OnClickListener {
        public void onClick(final View v) {
            ...
            final boolean afterKeyguardGone = intent.isActivity()
                    && PreviewInflater.wouldLaunchResolverActivity(mContext, intent.getIntent(),
                            mCurrentUserId);
             closePanel();     // wanggang add for notifyoverlaySecurity 0528               
            dismissKeyguardThenExecute(new OnDismissAction() {
                public boolean onDismiss() {
    ...
    //A: lanhua for hall_view 160422 ned
}
